...
bool FindDots(cv::Mat _im, std::vector<cv::Point> &_cog)
{
    ...
    // cv::waitKey(0);
    std::cout << "  - found " << contours_tmp.size() << " contours" << std::endl;
    ...
    std::cout << "  - eliminating outer/non-ellipsoids -> " << count << std::endl;
    ...
    {
        std::cout << "  -> Not enough candidates to find grid (" << candidates.size() << ")" << std::endl;
        return true;
    }
    ...
    std::cout << "  - kept only 36" << std::endl;
